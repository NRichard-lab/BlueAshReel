...
The only local listener is `127.0.0.1:<port>` (development default18080). Root/status access requires the Portal authorization flow. Unpaired devices display Unpaired; Connected appears only with a fresh authenticated relay connection. The native tray exposes Portal, local status, pair, reconnect, pause/resume, restart, status, logs, settings, login startup and confirmed exit. Closing a status/settings window leaves the tray running.
