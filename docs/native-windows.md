...
The same installer supports explicit `/ISOLATEDTEST=<id>` acceptance, where `id` is 1–32 lowercase letters/digits. Its paths are fixed to `%USERPROFILE%\BlueAshReel-Installer-Tests\<id>\program` and `data`; it requires a dedicated matching marker, uses a separate AppId and HKCU `Software\BlueReel\InstallerTests\<id>` registration, and rejects redirected or linked paths. This dedicated profile subdirectory avoids MSIX virtualization of LocalAppData when a packaged desktop tool launches the acceptance test. The mode creates no startup entry or shortcuts, launches no browser or tray automatically, and disables the test tray's startup controls. Uninstall recovers the same scope from the verified installed path and marker. Normal installation retains its LocalAppData defaults, existing development AppId and startup convention. **Never test the normal installer with only `/DIR` or `/DATADIR` on a machine with an existing installation.**
