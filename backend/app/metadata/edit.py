...
from app.models import MediaItem, MetadataRecord
...
def _row(db: Session, item: MediaItem) -> MetadataRecord:
...
def _provider_row(db: Session, item: MediaItem) -> MetadataRecord:
    row = _row(db, item)
...
def _sources(row: MetadataRecord, config: AppConfig, kind: str) -> tuple[MetadataProvider, list[ArtworkSource]]:
...
def artwork_candidates(db: Session, item: MediaItem, config: AppConfig, kind: str) -> dict[str, Any]:
...
def artwork_preview(db: Session, item: MediaItem, config: AppConfig, kind: str, candidate_id: str) -> dict[str, Any]:
    row = _provider_row(db, item)
    provider, sources = _sources(row, config, kind)
    try:
...
def select_artwork(db: Session, item: MediaItem, config: AppConfig, kind: str, candidate_id: str) -> dict[str, Any]:
    row = _provider_row(db, item)
    provider, sources = _sources(row, config, kind)
    try:
        source = next((s for s in sources if _key(s.provider_path) == candidate_id), None)
...
def restore_artwork(db: Session, item: MediaItem, kind: str) -> dict[str, Any]:
    row = _row(db, item)
    if kind not in {"poster", "background"}:
        raise ValueError("Invalid artwork kind")
    selections = dict(row.artwork_selections or {})
