...
def defaults(db: Session, item: MediaItem) -> dict[str, Any]:
...
def search(db: Session, item: MediaItem, config: AppConfig, title: object, year: object) -> dict[str, Any]:
    defaults(db, item)
...
def preview(db: Session, item: MediaItem, config: AppConfig, provider_id: object) -> dict[str, Any]:
    defaults(db, item)
    if not isinstance(provider_id, str) or not re.fullmatch(r"[1-9][0-9]{0,11}", provider_id):
        raise ValueError("Invalid TMDB identifier")
    provider = configured_provider(config)
